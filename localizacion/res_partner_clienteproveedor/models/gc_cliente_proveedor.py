...
class gc_cliente_proveedor(models.Model):
    ...
    def validate_num_cliente(self,valor):
        res = {}
        num_obj = re.compile(r"""^\d{4}""", re.X)
        if num_obj.search(valor):
            res = {
                'num_cliente':valor
            }
        return res
            ...
            if not self.validate_rif_duplicate(vals.get('vat', False)):
                raise exceptions.except_orm(('Advertencia!'),
                                            (u'El cliente o proveedor ya se encuentra registrado con el rif: %s') % (
                                                vals.get('vat', False)))
        if vals.get('num_cliente', False):
            res = self.validate_num_cliente(vals.get('num_cliente', False))
            if not res:
                raise exceptions.except_orm(('Advertencia!'),
                                                (u'Codigo de Cliente debe contener solo números. Ej. 1000'))
        ...
        if vals.get('vat'):
            ...
            if not self.validate_rif_duplicate(vals.get('vat', False), True):
                raise exceptions.except_orm(('Advertencia!'),
                    ...
                    vals.get('vat', False)))
            if vals.get('num_cliente'):
                res = self.validate_num_cliente(vals.get('num_cliente'))
                if not res:
                    raise exceptions.except_orm(('Advertencia!'),
                                                (u'Codigo de Cliente debe contener solo números. Ej. 1000'))
